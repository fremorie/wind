...
float roadCenterZ(vec2 position) {
    float riverMask = getRiverMask(position);
    float roadAmplitude = uRoadAmplitude * (1.0 - riverMask);

    return uRoadCenter.z + roadAmplitude * sin(position.x * uRoadWaviness);
}
...
float getRoadMask(vec2 position) {
    float distanceToRoad = abs(
        mod(position.y - roadCenterZ(position) + uRoadPeriod / 2.0, uRoadPeriod)
        - uRoadPeriod / 2.0
    );
    float roadMask = 1.0 - smoothstep(uRoadWidth - uRoadFalloff, uRoadWidth, distanceToRoad);
...
float getRoadElevation(vec2 position) {
    ...
    return getElevation(
        vec2(position.x, roadCenterZ(position))
    ) * roadFlatness;
}
